...
- **観点B**: タイトル・本文・設問が一読で分かる自然な日本語か。翻訳調・漢数字の数量表記は修正。各文について「主語→述語→因果」が追えるかを確認し、意味が取れない文、名詞をつないだだけの文、根拠記事にない補完は本文段落ごと書き直す。
- **観点B-2**: 明快さを1問ずつ1〜5で採点し **clarity-audit.json を必ず更新**（全問≥3かつavg≥4でpass。D系ゲートが検査）。
- **観点C**: 「誰かに話したくなる読み応え」があるか。凡庸なら差し替え。ユーザーの読後評価（バックアップに `contentFeedback` があれば参照）で高評価のジャンル・論点を優先し、低評価の型を繰り返さない。
